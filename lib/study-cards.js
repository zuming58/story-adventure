...
export function validateCardPayload(payload) {
  ...
  const targetLevel = normalizeTargetLevel(payload?.targetLevel);
  ...
  return { subject, grade, targetLevel, material };
}
...
function buildPrompt({ subject, grade, targetLevel, material }) {
  const levelInstruction =
    targetLevel === "全部"
      ? "难度要求：请混合生成简单、中等、较难卡片，并让难度分布适合复习。"
      : `难度要求：请尽量生成“${targetLevel}”难度的卡片。`;
...
function normalizeTargetLevel(level) {
  const value = String(level || "全部").trim();
  return ["全部", "简单", "中等", "较难"].includes(value) ? value : "全部";
}
