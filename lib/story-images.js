...
const DEFAULT_STYLE_PROMPT = [
  ...
  "整本故事书要保持同一套绘本画风：相同的线条粗细、柔和色彩、暖光氛围、干净背景和童话感构图。",
  "如果故事里已经出现主角、动物、怪物、机器人或伙伴，请尽量保持它们的外貌特征、服装颜色、体型比例和表情气质一致，不要每一幕换成新形象。",
  CHILD_SAFE_IMAGE_RULES,
].join("\n");
...
export async function generateStoryImage(payload, env = process.env) {
  ...
  const providers = getImageProviders(env, input.preferredProvider);
  const errors = [];
...
function getImageProviders(env, preferredProvider = "") {
  ...
  const normalizedPreferred = normalizeProviderName(preferredProvider);
  const prioritized = normalizedPreferred
    ? [normalizedPreferred, ...order.filter((name) => normalizeProviderName(name) !== normalizedPreferred)]
    : order;

  return prioritized.map((name) => createImageProvider(name, env)).filter(Boolean);
}
...
function createImageProvider(name, env) {
  ...
  if (name === "uu-fast" || name === "uu-gpt-fast") {
    ...
    return {
      ...
      model: env.UU_FAST_IMAGE_MODEL || env.IMAGE_MODEL || "gpt-image-2",
      size: env.UU_FAST_IMAGE_SIZE || env.IMAGE_SIZE || "1024x1024",
...
function validateImagePayload(payload) {
  ...
  const preferredProvider = normalizeProviderName(payload?.preferredProvider);
  ...
  return { title, text, imagePrompt, genre, actNumber, includeGuides, preferredProvider };
}
