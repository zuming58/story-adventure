...
export async function generateStoryAdventure(payload, env = process.env) {
  ...
  return input.mode === "ending" ? normalizeEnding(parsed) : normalizeScene(parsed);
}
...
async function requestAndValidateJson({ baseUrl, apiKey, model, prompt, schema, input }) {
  ...
  for (const attempt of attempts) {
    ...
    try {
      ...
      input.mode === "ending" ? normalizeEnding(parsed) : normalizeScene(parsed);
      return result.text;
    } catch (error) {
  ...
  const aiResponse = await fetch(`${baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "你只输出符合要求的 JSON，不输出 Markdown、解释或代码块。",
        },
...
function buildStoryPrompt(input) {
  const commonRules = [
    ...
    "必须严格承接孩子写的故事开头，保留其中已经出现的主角、地点、关键物品和事件，不能另起炉灶、不能换成新的故事开头。",
    "如果孩子的开头里有发光地图、旧钟楼、储物柜等具体元素，第一幕必须围绕这些元素继续推进。",
    "第一幕可以用孩子开头的内容做简短承接，但要继续写出新的发现、新的问题或新的选择。",
    "故事要训练孩子的因果逻辑和语文写作感：每一幕都要承接孩子的选择，并制造一个清楚的新问题。",
    "每一幕正文控制在 90-150 个中文汉字，适合手机阅读。",
    "插图提示词要适合绘本/漫画风格，描述画面主体、场景、情绪和色彩，不要出现文字排版要求。",
  ...
  if (input.mode === "ending") {
    return [
      ...
      "禁止凭空新增人名，例如前文没有出现“小林”，结局就不能写“小林”。无法判断同伴姓名时，只写“你和伙伴”或“你们”。",
      "如果前文核心角色是小兔子、小怪物、会说话的地图等，就围绕这些角色收束结局。",
      "结局必须解释或回收前文至少 2 个关键线索，不能突然改变地点、目标、主角关系或故事规则。",
      "结局正文控制在 120-180 个中文汉字。",
      "type 字段请写成类似“友情结局”“勇气结局”“智慧结局”“搞笑结局”的短标签。",
      "protagonist 字段写前文真实出现过的主角或角色组合，无法判断时写“你和伙伴”。",
    ].join("\n");
  }
  ...
  return [
    ...
    "新一幕必须读取并承接已生成故事全文，尤其是上一幕结尾和孩子刚才选择，不能跳到无关的新地点、新任务或新主角。",
    "每一幕都要保持同一个主线目标：延续开头的问题、地图/道具/角色线索和孩子选择造成的结果。",
